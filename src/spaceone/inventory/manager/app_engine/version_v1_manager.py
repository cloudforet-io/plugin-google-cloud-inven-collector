...
class AppEngineVersionV1Manager(GoogleCloudManager):
    ...
    def get_version(self, service_id: str, version_id: str, params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def get_version_metrics(self, service_id: str, version_id: str, params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def collect_cloud_service(
        ...
        """App Engine 버전 정보를 수집합니다 (v1 API)."""
        _LOGGER.debug(f"** App Engine Version V1 START **")
        ...
        for service in services:
            ...
            for version in versions:
                try:
                    ...
                    metrics = {}
                    if version_id:
                        metrics = self.get_version_metrics(service_id, version_id, params)
                    ...
                    if "automaticScaling" in version:
                        ...
                        version_data["automaticScaling"] = {
                            "coolDownPeriod": str(auto_scaling.get("coolDownPeriod", "")),
                            "cpuUtilization": auto_scaling.get("cpuUtilization", {}),
                            "maxConcurrentRequests": auto_scaling.get("maxConcurrentRequests"),
                    ...
                    app_engine_version_data = AppEngineVersion(version_data, strict=False)
                    ...
                    error_responses.append(
                        self.generate_error_response(e, self.cloud_service_group, "Version")
                    )
        
        _LOGGER.debug(f"** App Engine Version V1 END **")
