...
from spaceone.inventory.model.pub_sub.snapshot.cloud_service import SnapshotResource, SnapshotResponse
    ...
    def collect_cloud_service(self, params):
        ...
        for snapshot in snapshots:
            try:
                ...
                ##################################
                collected_cloud_services.append(SnapshotResponse({'resource': snapshot_resource}))

            except Exception as e:
                _LOGGER.error(f'[collect_cloud_service] => {e}', exc_info=True)
                error_response = self.generate_resource_error_response(e, 'Pub/Sub', 'Snapshot', snapshot_id)
                error_responses.append(error_response)

        _LOGGER.debug(f'** Pub/Sub Snapshot Finished {time.time() - start_time} Seconds **')
