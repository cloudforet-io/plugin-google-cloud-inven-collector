...
class BatchJobManager(GoogleCloudManager):
    ...
    def _process_single_job(self, job: Dict, batch_conn: BatchV1Connector) -> Dict:
        """
        개별 Job을 처리합니다.
    ...
    def _process_single_task_group(
        self, task_group: Dict, machine_type: str, batch_conn: BatchV1Connector
    ) -> Dict:
        """
        개별 TaskGroup을 처리합니다.
    ...
    def _collect_tasks_safe(
        self, task_group_name: str, batch_conn: BatchV1Connector
    ) -> List[Dict]:
        """
        Tasks를 안전하게 수집합니다.
