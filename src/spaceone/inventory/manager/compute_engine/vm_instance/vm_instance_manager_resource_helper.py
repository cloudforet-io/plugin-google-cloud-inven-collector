...
from spaceone.inventory.model.compute_engine.instance.data import (
    Compute,
    GoogleCloud,
    OS,
    Hardware,
...
class VMInstanceManagerResourceHelper(GoogleCloudManager):
    ...
    def _get_appropriate_image_info(os_identity, licenses, public_images):
                ...
                for image in images:
                    if licenses == image.get("licenses", []):
                        ...
                        os_data.update(
                            {
                                "os_distro": (
                                    "windows-server" if key == "windows" else key
                                ),
                                "details": image.get("description", ""),
                                "os_arch": (
                                    arch_list[os_arch_index[0]]
        ...
        if os_identity == "cos":
            os_data.update(
                {"os_distro": "cos", "details": "Google, Container-Optimized OS"}
            )
        return os_data
    ...
    def _get_ssh_keys(items):
        ssh_keys_info = {"ssh_keys": [], "block_project_ssh_keys": "OFF"}
        if items:
            for item in items:
