...
subscription_detail = ItemDynamicLayout.set_fields(
    ...
    fields=[
        ...
        TextDyField.data_source(
            "Subscription expiration", "data.display.subscription_expiration"
        ),
        TextDyField.data_source(
            "Acknowledgement deadline", "data.display.ack_deadline_seconds"
        ),
        TextDyField.data_source("Subscription filter", "data.filter"),
        TextDyField.data_source(
            "Subscription message retention duration", "data.display.retention_duration"
        ),
        TextDyField.data_source(
            "Retain acknowledged messages", "data.display.retain_acked_messages"
        ),
        TextDyField.data_source(
        ...
        TextDyField.data_source(
            "Dead letter topic", "data.dead_letter_policy.dead_letter_topic"
        ),
        TextDyField.data_source(
            "Maximum delivery attempts", "data.dead_letter_policy.max_delivery_attempts"
        ),
        TextDyField.data_source(
            "Retry policy", "data.display.retry_policy.description"
        ),
        TextDyField.data_source(
            "Minimum backoff duration", "data.display.retry_policy.minimum_backoff"
        ),
        TextDyField.data_source(
            "Maximum backoff duration", "data.display.retry_policy.maximum_backoff"
        ),
        TextDyField.data_source("Push endpoint", "data.push_config.push_endpoint"),
    ],
)
subscription_detail_meta = ListDynamicLayout.set_layouts(
    "Details", layouts=[subscription_detail]
)
...
class SubscriptionResource(PubSubResource):
    ...
    _metadata = ModelType(
        CloudServiceMeta, default=subscription_meta, serialized_name="metadata"
    )
