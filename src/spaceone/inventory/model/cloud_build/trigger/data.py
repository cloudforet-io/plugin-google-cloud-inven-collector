...
class Trigger(Model):
    ...
    filter = StringType()
    trigger_template = DictType(BaseType, deserialize_from="triggerTemplate", default={})
    ...
    repository_event_config = DictType(BaseType, deserialize_from="repositoryEventConfig", default={})
    build = DictType(BaseType, default={})
    autodetect = BooleanType(default=False)
    create_time = StringType(deserialize_from="createTime")  # DateTimeType 대신 StringType 사용
