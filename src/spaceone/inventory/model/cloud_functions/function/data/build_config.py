...
class RepoSource(Model):
    ...
    invert_regex = BooleanType(serialize_when_none=False, deserialize_from='invertRegex')
...
class Source(Model):
    storage_source = ModelType(StorageSource, serialize_when_none=False, deserialize_from='storageSource')
    repo_source = ModelType(RepoSource, serialize_when_none=False, deserialize_from='repoSource')


class SourceProvenance(Model):
    resolved_storage_source = ModelType(StorageSource, serialize_when_none=False,
                                        deserialize_from='resolvedStorageSource')
    resolved_repo_source = ModelType(RepoSource, serialize_when_none=False, deserialize_from='resolvedRepoSource')
...
class BuildConfig(Model):
    ...
    source_provenance = ModelType(SourceProvenance, serialize_when_none=False, deserialize_from='sourceProvenance')
