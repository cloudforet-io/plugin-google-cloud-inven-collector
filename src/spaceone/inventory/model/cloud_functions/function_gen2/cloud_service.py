...
general_information = ItemDynamicLayout.set_fields(
    ...
    fields=[
        ...
        TextDyField.data_source(
            "Minimum instances", "data.service_config.min_instance_count"
        ),
        TextDyField.data_source(
            "Maximum instances", "data.service_config.max_instance_count"
        ),
        TextDyField.data_source(
            "Service account", "data.service_config.service_account_email"
        ),
...
networking_settings = ItemDynamicLayout.set_fields(
    ...
    fields=[
        ...
        TextDyField.data_source(
            "VPC connector egress", "data.display.vpc_connector_egress"
        ),
    ],
)
...
https = ItemDynamicLayout.set_fields(
    "HTTPS", fields=[TextDyField.data_source("URL", "data.service_config.uri")]
)
eventarc_trigger = ItemDynamicLayout.set_fields(
    "Eventarc trigger",
    fields=[
        ...
        TextDyField.data_source(
            "Receive events from", "data.event_trigger.pubsub_topic"
        ),
        TextDyField.data_source("Trigger region", "data.event_trigger.trigger_region"),
        TextDyField.data_source(
            "Service account", "data.event_trigger.service_account_email"
        ),
        TextDyField.data_source("Retry on failure", "data.display.retry_policy"),
    ],
)
function_trigger_meta = ListDynamicLayout.set_layouts(
    "Trigger", layouts=[https, eventarc_trigger]
)
