...
class FunctionGen1(BaseResource):
    ...
    build_environment_variables = DictType(StringType, serialize_when_none=False,
                                           deserialize_from='buildEnvironmentVariables')
    network = StringType(serialize_when_none=False)
    max_instances = IntType(serialize_when_none=False, deserialize_from='maxInstances', default=0)
    min_instances = IntType(serialize_when_none=False, deserialize_from='minInstances', default=0)
    ...
    build_worker_pool = StringType(serialize_when_none=False, deserialize_from='buildWorkerPool')
    ...
    secret_environment_variables = ListType(ModelType(SecretEnvVar), serialize_when_none=False,
                                            deserialize_from='secretEnvironmentVariables')
    secret_volumes = ListType(ModelType(SecretVolume), serialize_when_none=False,
                              deserialize_from='secretVolumes')
    source_token = StringType(serialize_when_none=False, deserialize_from='sourceToken')
    docker_repository = StringType(serialize_when_none=False, deserialize_from='dockerRepository')
    docker_registry = StringType(
        choices=('DOCKER_REGISTRY_UNSPECIFIED', 'CONTAINER_REGISTRY', 'ARTIFACT_REGISTRY'),
        serialize_when_none=False,
        deserialize_from='dockerRegistry')
    source_archive_url = StringType(serialize_when_none=False, deserialize_from='sourceArchiveUrl')
    source_repository = ModelType(SourceRepository, serialize_when_none=False,
                                  deserialize_from='sourceRepository')
    source_upload_url = StringType(serialize_when_none=False, deserialize_from='sourceUploadUrl')
    https_trigger = ModelType(HttpsTrigger, serialize_when_none=False,
                              deserialize_from='httpsTrigger')
    event_trigger = ModelType(EventTrigger, serialize_when_none=False,
                              deserialize_from='eventTrigger')
