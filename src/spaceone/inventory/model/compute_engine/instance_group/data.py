...
class AutoScalerStatusDetail(Model):
    message = StringType()
    type = StringType(choices=('ALL_INSTANCES_UNHEALTHY', 'BACKEND_SERVICE_DOES_NOT_EXIST',
                               ...
                               'MISSING_LOAD_BALANCING_DATA_POINTS', 'MODE_OFF', 'MODE_ONLY_UP',
                               'MORE_THAN_ONE_BACKEND_SERVICE', 'NOT_ENOUGH_QUOTA_AVAILABLE',
...
class InstanceGroupManagerStatusStateful(Model):
    ...
    per_instance_configs = ModelType(InstanceGroupManagerStatusStatefulPerInstanceConfig,
...
class InstanceGroupManagerStatus(Model):
    ...
    version_target = ModelType(InstanceGroupManagerStatusVersionTarget, deserialize_from='versionTarget')
...
class InstanceGroupManagerVersion(Model):
    ...
    target_size = ModelType(InstanceGroupManagerVersionTargetSize, deserialize_from='targetSize')
...
class InstanceGroupManagerUpdatePolicy(Model):
    ...
    instance_redistribution_type = StringType(choices=('PROACTIVE', 'NONE'),
                                              deserialize_from='instanceRedistributionType')
    minimal_action = StringType(choices=('RESTART', 'REPLACE', 'RESTART'), deserialize_from='minimalAction')
...
class InstanceGroupManagerStatefulPolicy(Model):
    preserved_state = ModelType(InstanceGroupManagerStatefulPolicyPreservedState, deserialize_from='preservedState')
...
class InstanceGroupManagers(Model):
    ...
    target_pools = ListType(StringType, deserialize_from='targetPools', serialize_when_none=False)
    fingerprint = StringType()
    current_actions = ModelType(InstanceGroupManagerCurrentAction, deserialize_from='currentActions')
    status = ModelType(InstanceGroupManagerStatus)
    target_size = IntType(deserialize_from='targetSize')
    self_link = StringType(deserialize_from='selfLink')
    auto_healing_policies = ListType(ModelType(InstanceGroupManagerAutoHealingPolicy),
                                     deserialize_from='autoHealingPolicies')
    update_policy = ModelType(InstanceGroupManagerUpdatePolicy,
    ...
    stateful_policy = ModelType(InstanceGroupManagerStatefulPolicy,
