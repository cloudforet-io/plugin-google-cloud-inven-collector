...
class TargetProxy(Model):
    name = StringType(serialize_when_none=False)
    type = StringType(choices=['GRPC', 'HTTP', 'HTTPS', 'SSL', 'TCP'], default='TCP', serialize_when_none=False)
...
class ForwardingRule(Model):
    ...
    ip_protocol = StringType(deserialize_from='IPProtocol', choices=['TCP', 'UDP', 'ESP', 'AH', 'SCTP', 'ICMP',
                                                                     'L3_DEFAULT'], serialize_when_none=False)
    ...
    self_link = StringType(deserialize_from='selfLink', serialize_when_none=False)
    load_balancing_scheme = StringType(choices=['EXTERNAL', 'EXTERNAL_MANAGED', 'INTERNAL', 'INTERNAL_MANAGED',
                                                'INTERNAL_SELF_MANAGED'], serialize_when_none=False)
    subnetwork = StringType(serialize_when_none=False)
    network = StringType(serialize_when_none=False)
    backend_service = StringType(deserialize_from='backendService', serialize_when_none=False)
    service_label = StringType(deserialize_from='serviceLabel', serialize_when_none=False)
    ...
    all_global_access = BooleanType(deserialize_from='allowGlobalAccess', serialize_when_none=False)
    creation_timestamp = DateTimeType(deserialize_from='creationTimestamp', serialize_when_none=False)
...
class BackendService(Model):
    ...
    failover_policy = ModelType(FailOverPolicy, deserialize_from='failoverPolicy', serialize_when_none=False)
    ...
    connection_draining = ModelType(ConnectionDraining, deserialize_from='connectionDraining',
                                    serialize_when_none=False)
...
class BackEndBucket(Model):
    ...
    custom_response_headers = ListType(StringType(), default=[], deserialize_from='customResponseHeaders',
                                       serialize_when_none=False)
    creation_timestamp = DateTimeType(deserialize_from='creationTimestamp')
...
class TargetPools(Model):
    ...
    session_affinity = StringType(deserialize_from='sessionAffinity', choices=['NONE', 'CLIENT_IP', 'CLIENT_IP_PROTO'],
                                  serialize_when_none=False)
    failover_ratio = FloatType(deserialize_from='failoverRatio', serialize_when_none=False)
...
class Managed(Model):
    ...
    domain_status = ModelType(Labels, deserialize_from='domainStatus', serialize_when_none=False)
...
class Certificates(Model):
    ...
    subject_alternative_names = ListType(StringType(), default=[],
...
class UrlMap(Model):
    ...
    description = StringType(serialize_when_none=False)
    self_link = StringType(deserialize_from='selfLink', serialize_when_none=False)
    host_rule = ListType(ModelType(HostRule), deserialize_from='hostRules', serialize_when_none=False)
    creation_timestamp = DateTimeType(deserialize_from='creationTimestamp')
...
class LoadBalancing(Model):
    ...
    type = StringType(choices=['HTTP(S) LoadBalancer', 'HTTP(S) LoadBalancer(classic)', 'SSL Proxy LoadBalancer',
                      ...
                      serialize_when_none=False)
    protocol = StringType(choices=['HTTP', 'HTTPS', 'TCP', 'UDP', 'ESP', 'AH', 'SCTP', 'ICMP', 'L3_DEFAULT', 'UnKnown'],
                          serialize_when_none=False)
    ...
    legacy_health_checks = ListType(ModelType(LegacyHealthCheck), serialize_when_none=False)
