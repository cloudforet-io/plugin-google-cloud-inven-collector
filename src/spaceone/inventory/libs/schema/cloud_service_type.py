...
class CloudServiceTypeMeta(BaseMetaData):
    ...
    def set_fields(cls, name="", fields=[]):
        _table = MetaDataViewTable(
            {"layout": QuerySearchTableDynamicLayout.set_fields(name, fields)}
        )
        return cls({"view": MetaDataView({"table": _table})})

    @classmethod
    def set_meta(cls, name="", fields=[], search=[], widget=[]):
        table_meta = MetaDataViewTable(
            {"layout": QuerySearchTableDynamicLayout.set_fields(name, fields)}
        )
        return cls(
            {
                "view": MetaDataView(
                    {"table": table_meta, "search": search, "widget": widget}
                )
            }
        )
...
class CloudServiceTypeResource(Model):
    ...
    _metadata = PolyModelType(
        CloudServiceTypeMeta, serialize_when_none=False, serialized_name="metadata"
    )
